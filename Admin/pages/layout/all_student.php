                 ...
                 <form class="" action="all_student.php" method="post">
                   <div class="form-group ">
                     <select name="batch_id" class="form-control" >
                       ...
                       <option value="">Select one BATCH:</option>
                                      ...
                                      <?php
                                        include_once "../../../connection.php";
                                            ...
                                            $sql = "SELECT current_student.batch_id,batch.status,count(*) AS counter FROM current_student LEFT JOIN batch ON current_student.batch_id=batch.b_id group by batch_no";
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                              ...
                                              while($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo '<option value="'.$row['batch_id'].'">'.$row['batch_id'].'('.$row['status'].')'.'['.$row['counter'].']</option>'  ;
...
<div class="col-md-1">
             ...
             <div class="col-md-4 col-sm-4">
                 <div class="form-group ">
              <input type="text" name="search" class="form-control" value="" PLACEHOLDER="ID/name/college/mobile">
                     </div>
             </div>
                ...
                include_once "../../../connection.php";
                    ...
                    if(isset($_POST['batch_id'])){
                        $batch_id=$_POST['batch_id'];
                          $sql = "SELECT * FROM current_student where batch_id=$batch_id";
                    }else if(isset($_POST['search'])){
                            $search=$_POST['search'];
                            $sql = "SELECT * FROM current_student where s_name like '%$search%' or college like '%$search%' or roll like '%$search%' or phone_no like '%$search%' or nick_name like '%$search%'";

                        }else{
                          $sql = "SELECT * FROM current_student order by current_student_id desc";
                        }
                    ...
                    if (mysqli_num_rows($result) > 0) {
                      ...
                      while($row = mysqli_fetch_assoc($result)) {
                        echo "<tr class='info'>";
                          echo "<td> ID::" . $row["roll"]."<br>". $row["s_name"]."(".$row["nick_name"].")<br>".$row["college"]."<br>Class:".$row["class"]."<br>0".$row["phone_no"]."</td>";


                          echo "<td>" . $row["father_name"]."<br>0".$row["father_mobile"]."<br>".$row["f_profession"]."<br>"."";
                          echo "" . $row["mother_name"]."<br>".$row["mother_mobile"]."<br>".$row["m_profession"]."<br>"."</td>";
                          echo "<td>Course type: " . $row["type"]."<br>Course: ".$row["requested_course"]."<br>Shift: ".$row["shift"]."</td>";
                          echo "<td>" . $row["status"]."</br>Batch no:".$row["batch_id"]."</td>";
                          echo '<td> <img  height="100px" src="../../../application/'.$row["image"].'"</td>';
                          echo '<td><a onclick="return check();" href="delete_student.php?id='.$row['current_student_id']. '"class="btn btn-info btn-sm">
          <span class="glyphicon glyphicon-remove"></span>
        </a>
        <a href="edit_student.php?id='.$row['current_student_id']. '"class="btn btn-info btn-sm">
<span class="glyphicon glyphicon-pencil"></span>
</a>
        <a href="change_status_student.php?id='.$row['current_student_id']. '"class="btn btn-info btn-sm">
<span class="glyphicon ">Change status</span>
</a>
</br>
</br>
<a href="change_batch.php?id='.$row['current_student_id']. '"class="btn btn-info btn-sm">
<span class="glyphicon ">Change batch</span>
</a>
<a href="payment_status.php?id='.$row['current_student_id']. '"class="btn btn-info btn-sm">
<span class="glyphicon ">Payment</span>
</a>
</td>';
                        echo "</tr>";
                      }
