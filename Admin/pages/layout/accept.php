...
if (mysqli_num_rows($result) > 0) {
    ...
    if($row = mysqli_fetch_assoc($result)) {
        ...
        $course=$row['course'];
        $mayment_method=$row['mayment_method'];
        $pay_id=$row['pay_id'];
        $amount=$row['amount'];
        $type=$row['type'];
        $image=$row['image'];
...
$sql = "INSERT INTO current_student (s_name,nick_name,college,class,phone_no,shift,father_mobile,mother_mobile,father_name,f_profession,mother_name,m_profession,requested_course,type,status,image)
VALUES ('$s_name','$nick_name','$college', '$class','$phone_no','$shift','$father_mobile','$mother_mobile','$father_name','$f_profession',
'$mother_name','$m_profession','$course','$type','active','$image')";
...
// find out last id of current student
$sql = "SELECT current_student_id FROM current_student ORDER BY current_student_id DESC LIMIT 1";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        $s_id=$row['current_student_id'];
    }
} else {
    $s_id=1;
}
...
//payable amount
$sql = "SELECT * FROM payment_set ";
$result = mysqli_query($conn, $sql);
  $payable=0;
if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        if($row['type']==$type && $row['course']==$course & $row['shift']==$shift){
            $payable=$row['amount'];
        }
    }
} else {
...
if($amount>0){
  ...
  VALUES ('$s_id', '$mayment_method', '$pay_id','$amount','$date')";
